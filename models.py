...
class Gen(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=3, n_resblocks=9, norm=False):
        ...
        for _ in range(n_resblocks):
            model = model + [ResBlock(in_features)]
...
class Attn(nn.Module):
    ...
    def __init__(self, input_nc=3):
        ...
        model = model + [  nn.Conv2d(32, 64, 3, stride=2, padding=1),
                    nn.InstanceNorm2d(64),
                    nn.ReLU(inplace=False) ]

        model = model + [ResBlock(64, norm=True)]

        model = model + [nn.UpsamplingNearest2d(scale_factor=2)]

        model = model + [  nn.Conv2d(64, 64, 3, stride=1, padding=1),
                    nn.InstanceNorm2d(64),
                    nn.ReLU(inplace=False) ]
        
        # model += [nn.UpsamplingNearest2d(scale_factor=2)]

        model = model + [  nn.Conv2d(64, 32, 3, stride=1, padding=1),
                    nn.InstanceNorm2d(32),
                    nn.ReLU(inplace=False) ]

        model = model + [  nn.Conv2d(32, 1, 7, stride=1, padding=3),
                    nn.Sigmoid() ]
